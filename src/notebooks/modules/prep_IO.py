...
def get_sample_hosts( n, search_set=arc_data ):
    out = []
...
def collect_ids_hostfile( host_gzfile ):
    lines = unzip_txt( host_gzfile )
    full_list = [ line for line in lines if 'comet' in line and ' - ' not in line ]
    ret_list = list({ line.split(" ")[1]:0 for line in full_list[1:] }.keys() )
    return ret_list
...
####Formatting
def purge_char( blob, char, repl ):
    if char in blob:
        temp = blob.replace( char, repl )
        return purge_char( temp, char, repl )
    else:
        return blob

def purge_all_char( token, char, repl='' ):
    if char in token:
        return purge_char( token, char, repl )
    else:
        return token
...
def labeled_data( data_list, times=True ):
    out_dict = {}
...
def host_to_info_dict( zip_txt, data_only=False, jobid=0 ):
    ...
    for j in range( len(contents) ):
        ...
        if "%" in line.split(" ")[0] or line[0] in ['$', '!']:
            next
        
        elif line == '/n' or len( line ) < 3:
            next
        ...
        # Data line
            ...
            dev = chunks[1]
            data = chunks[2:-1]
            data.append( chunks[-1][:-1] )
            
            for k in range(len( data ) ):
                metric = info_dict["Schemas"][ stat ][ k ]
                ...
                info_dict["Data"].append( (stat, dev, metric, datum, t, jobid) )
...
def hostgz_to_X_data( host_file_list ):
    
    for i in range(len( host_file_list )):
        host_file, x = try_open_x( host_file_list, i )
        i = x
        
        name_idx = host_file.find("comet-")
        host_name = host_file[ name_idx : name_idx + 11 ]
        
        file_info = host_to_info_dict( host_file )
        host_data = file_info["Data"]
        
        base = labeled_data( filter_for_id( host_data, "-" ), times=False )
        jobids = file_info["Jobid(s)"]
        
        for j in range(len( jobids )):
            sample_id = jobids[j]
            sample_token = prep_fs.id_encoder( sample_id, host_name )
            sample_dataset = labeled_data( filter_for_id( host_data, sample_id ), times=False )
            features = list( sample_dataset.keys() )
                        
            sample_data = {}
            for key,val in sample_dataset.items():               
                if len(val) == 1 and type(val) == list and type(val[0]) == list:
                    sample_data[key] = val[0]
                else:
                    sample_data[key] = val
                        
            for k in range(len( features )):
                sample_feature = features[k]
                
                clean_stat = purge_all_char( sample_feature[0], '/' )
                clean_dev = purge_all_char( sample_feature[1], '/' )
                clean_metric = purge_all_char( sample_feature[2], '/' )
                
                out_data = { 'sample_token': sample_token, 'data': sample_data[ sample_feature ] }
                
                f_label = f"./data/{clean_stat}--{clean_dev}--{clean_metric}_data.csv"
                with open( f_label, 'a', newline='\n' ) as f:
                    
                    # If feature has not been saved previously, print feature to top of file
                    # then write token, data
                    if stat( f_label ).st_size == 0:
                        csv_writer = DictWriter( f, fieldnames=['feature', 'sample_token', 'data'] )
                        csv_writer.writerow( { "feature": sample_feature } )
                        csv_writer.writerow( out_data ) 
                    
                    # If feature has been saved but data refers to empty '-' jobid,
                    # overlook and write "duplicate" token with unique data
                    elif sample_id == '-':
                        csv_writer = DictWriter( f, fieldnames=['sample_token', 'data'] )
                        csv_writer.writerow( out_data )      
                    
                    # If feature has been saved and we have unique token, data
                    # verify token has not already been saved then write token, data
                    else:
                        with open( f_label , "r+b" ) as mfile:
                            s = mmap.mmap( mfile.fileno(), 0, prot=mmap.PROT_READ )
                            
                            if s.find( sample_token.to_bytes(16,'big') ) < 0:
